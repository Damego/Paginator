...
from discord_components import Button, ButtonStyle, ComponentMessage, Interaction
...
class Paginator:
    def __init__(
        self, bot: Union[Client, Bot], ctx: Context, style: int, embeds: List[Embed]
    ) -> None:
        self.bot = bot
        self.ctx = ctx
        self.style = style
    ...
    async def start(self):
        self.message: Union[ComponentMessage, Message] = await self.ctx.send(
    ...
    async def _get_interaction(self) -> Interaction:
        try:
